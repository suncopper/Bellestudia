...
document.addEventListener('DOMContentLoaded', () => {
  ...
  btnGenerate.addEventListener('click', async () => {
    ...
    try {
      ...
      if (jsonData.questions) jsonData.questions.forEach(q => { processItem(q); if (!q.id) q.id = generateUUID(); });
      if (jsonData.statements) jsonData.statements.forEach(s => { processItem(s); if (!s.id) s.id = generateUUID(); });
      if (jsonData.items) jsonData.items.forEach(i => { processItem(i); if (!i.id) i.id = generateUUID(); });
